...
> **Note (Leuro™ adaptation):** The Leuro™ App applies this structural system — typography scale, spacing, 44px card / 99px pill radii, and dual-layer shadows — using Leuro's own brand colors instead of the palette below: Twilight Purple `#5A3E76` (primary/headers), Eureka Gold `#E6B347` (accent/CTAs), Oat Canvas → `#F9F9F7`, body text `#363636`, borders `#E0D8EC`. See `style.css` for the resulting `:root` tokens.
